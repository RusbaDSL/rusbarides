...
export default function AppCommon({ children }) {
  ...
  // Audio status listeners - automatically update playing state and handle completion
  useEffect(() => {
    // Horn player status listener
    const hornStatusListener = (status) => {
      setHornPlayerPlaying(status.playing);
      
      // Reset player when audio completes
      if (status.duration > 0 && (
          (!status.playing && status.currentTime >= status.duration) ||
          (status.playbackRate === 0 && status.currentTime >= status.duration) ||
          (status.playing && status.currentTime >= status.duration && Math.abs(status.currentTime - status.duration) < 0.1)
      )) {
  ...
  const notifyBooking = async () => {
    for(let i=0;i<tasks.length;i++){
      if(!playedSounds.includes(tasks[i].id)){
        ...
        if(settings && settings.CarHornRepeat){
          // Force reset if player seems stuck
