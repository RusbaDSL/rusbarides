...
import { ExtraInfo, RateView, appConsts, MAIN_COLOR, MAIN_COLOR_DARK, SECONDORY_COLOR } from '../common/sharedFunctions';
        ...
        if (bookinglistdata.bookings) {
            ...
            for (let i = 0; i < bookinglistdata.bookings.length; i++) {
                if (bookinglistdata.bookings[i].status === 'PAID' || bookinglistdata.bookings[i].status === 'COMPLETE') {
                    const { tripdate, driver_share } = bookinglistdata.bookings[i];
